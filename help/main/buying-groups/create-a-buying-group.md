---
title: Créer un groupe d'achat
description: Découvrez comment utiliser une audience de compte, un modèle de rôle et un intérêt pour une solution afin de créer des groupes d’achat.
feature: Buying Groups
role: Admin, User
level: Beginner, Intermediate, Experienced
doc-type: Feature Video
duration: 480
last-substantial-update: 2024-08-23T00:00:00Z
jira: KT-16054
exl-id: 4c478b95-f3b9-4ef4-ab8f-4cbb31650628
source-git-commit: c7b76b0f10bc4becb38c515e0b4c4cc9c457c6fb
workflow-type: tm+mt
source-wordcount: '44'
ht-degree: 0%

---

# Créer un groupe d&#39;achat

Découvrez comment utiliser une audience de compte, un modèle de rôle et un intérêt pour une solution afin de créer des groupes d’achat.

>[!VIDEO](https://video.tv.adobe.com/v/3451763/?captions=fre_fr&learn=on)
